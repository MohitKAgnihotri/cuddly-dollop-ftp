...
bool TcpClient::getfile(const char *filename) {
    ...
    while (rpdu.length == BUFFER_LENGTH) {
        ...
        if (rpdu.type == DATA_SUCCESS)
            ...
            printf("Unexpected Message from the server: MessageType = [%d], Message "
                   "content = [%s]\n",
                   rpdu.type, rpdu.buffer);
            isError = true;
            break;
        }
    }
...
void TcpClient::run(int argc, char *argv[]) {
    ...
    ServAddr.sin_addr.s_addr =
            ResolveName(inputserverhostname); /* Server IP address */
    ...
    switch (inputrequesttype) {
        case GET:
            ...
            if (rpdu.type == DATA_SUCCESS) {
                ...
                err_sys("Unexpected Message from the server: MessageType = [%d], Message "
                        "content = [%s]\n",
                        rpdu.type, rpdu.buffer);
            }
            break;
        ...
        case GETALL:
            ...
            if (CreateDirectory(string("Client_" + string(inputfoldername)).c_str(),
                                NULL)) {
                std::cout << "Directory"
                          << string("Client_" + string(inputfoldername)).c_str()
            ...
            if (rpdu.type == DATA_SUCCESS) {
                ...
                for (auto &file : fileList) {
                    ...
                    if (rpdu.type == DATA_SUCCESS) {
                        ...
                        err_sys("Unexpected Message from the server: MessageType = [%d], "
                                "Message content = [%s]\n",
                                rpdu.type, rpdu.buffer);
                    }
                }
...
void TcpClient::err_sys(const char *fmt,
...
int TcpClient::msg_recv(int sock, Msg *msg_ptr) {
    ...
    for (rbytes = 0; rbytes < msg_ptr->length; rbytes += n)
        if ((n = recv(sock, static_cast<char *>(msg_ptr->buffer) + rbytes,
...
int main(int argc,
         char *argv[]) // argv[1]=servername argv[2]=filename argv[3]=time/size
{
