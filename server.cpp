...
//////////////////////////////TcpThread Class //////////////////////////////////////////
void
TcpThread::err_sys(const char *fmt, ...)
{
    perror(NULL);
    va_list args;
        va_start(args, fmt);
...
unsigned long
TcpThread::ResolveName(char name[])
{
...
*/
int
TcpThread::msg_recv(int sock, Msg *msg_ptr)
{
    int rbytes, n;
...
*/
int
TcpThread::msg_send(int sock, Msg *msg_ptr)
{
    int n;
...
void
TcpThread::Create_Ready_Request(Msg &pdu)
{
...
void
TcpThread::Create_END_Request(Msg &pdu)
{
    memset(&pdu, 0x00, sizeof(Msg));
    pdu.type = END;
}
...
void
TcpThread::Create_Error_Request(Msg &pdu, const std::string &errorString)
{
    memset(&pdu, 0x00, sizeof(Msg));
    pdu.type = DATA_ERROR;
...
void
TcpThread::GetFileFromClient(std::fstream &fileToGet)
{
    Msg rpdu;
    do
    {
...
void
TcpThread::run() //cs: Server socket
{
    ...
    while (1)
    {
        ...
        {
            /* Download request */
            case GET:cout << "Starting transfer of" << rpdu.buffer << endl;
                ...
                /* Upload request */
            case PUT:Create_Ready_Request(spdu);
            ...
            case SYNC:
                ...
                if (isDirectoryExist(rpdu.buffer))
                {
                    ...
                    vector<filename_with_modTime_t *> filesinDirwithModTime = getFilesinDirWithModTime(rpdu.buffer);

                    if (!filesinDirwithModTime.empty())
                    {
                        // Send Ready  to the client
                        Create_Ready_Request(spdu);

                        // Start GET ALL for the all the files
                        for (auto file : filesinDirwithModTime)
                        {
                            strncat(spdu.buffer, file->filename.c_str(), file->filename.length());
                            strncat(spdu.buffer, ",", 1);
                            strncat(spdu.buffer, to_string(file->modTime).c_str(), to_string(file->modTime).length());
                            strncat(spdu.buffer, ",", 1);
                        }
                        ...
                        while (true)
                        {
                            ...
                            else if (rpdu.type == PUT)
                            {
                                file_put.open(rpdu.buffer, std::fstream::out | ios::binary);
                                if (!file_put.is_open())
                                    err_sys("failed to open the file");
            ...
            case PUTALL:
                ...
                {
                    std::cout << "Directory" << FolderName << " Sucessfully Created." << std::endl;
                    Create_Ready_Request(spdu);
                    msg_send(cs, &spdu); // tell client ready
                ...
                {
                    std::cout << "Directory" << FolderName << " Already Exist. Can't put." << std::endl;
...
int
TcpThread::SendFileToClient(fstream &file_get)
{
    Msg spdu;
